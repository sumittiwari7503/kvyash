...
export default function Navbar() {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-premium duration-300 bg-white border-b border-slate-100 ${
          scrolled ? "shadow-premium py-2.5" : "py-3"
        }`}
      >
          ...
          <div className="flex items-center justify-between h-14">
              ...
              {headerNavItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`text-sm font-medium transition-premium hover:text-brand-500 ${
            ...
            <div className="flex md:hidden">
              <button
                ...
                className="inline-flex items-center justify-center p-2 rounded-md text-slate-600 hover:text-brand-500 hover:bg-slate-50 focus:outline-none transition-premium"
      ...
      <div
        id="mobile-menu"
        className={`fixed top-0 right-0 bottom-0 z-[60] w-full max-w-sm bg-white p-6 shadow-xl md:hidden transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            type="button"
            onClick={() => setIsOpen(false)}
            className="p-2 rounded-md text-slate-600 hover:text-brand-500 focus:outline-none"
