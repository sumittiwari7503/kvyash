    ...
    <div className="font-sans text-navy-900 dark:text-slate-100 bg-white dark:bg-[#090d16] transition-colors duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {aiServices.map((svc) => {
              ...
              return (
                <div
                  key={svc.title}
                  className="bg-white dark:bg-[#0d1321] border border-slate-100 dark:border-slate-800 rounded-xl p-6 shadow-premium hover:shadow-premium-hover transition-premium flex flex-col justify-between"
                >
                  <div className="flex flex-col gap-4">
                    <div className="inline-flex self-start items-center justify-center p-3 rounded-lg bg-brand-50 dark:bg-slate-900 text-brand-500 dark:text-brand-400">
                      <Icon className="h-6 w-6" aria-hidden="true" />
                    </div>
                    <h4 className="text-lg font-bold text-navy-900 dark:text-slate-100">{svc.title}</h4>
                    <p className="text-slate-500 dark:text-slate-400 text-xs leading-relaxed">{svc.desc}</p>
                  </div>
                </div>
              );
            })}
      ...
      <section className="py-24 bg-slate-50 dark:bg-[#0c1321]/50 border-b border-slate-100 dark:border-slate-800/80 reveal-on-scroll">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="flex flex-col gap-6">
              ...
              <ul className="flex flex-col gap-3 mt-2">
                {capabilities.map((cap) => (
                  <li key={cap} className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400">
            ...
            <div className="bg-white dark:bg-[#0d1321] border border-slate-100 dark:border-slate-805 rounded-xl p-8 shadow-premium flex flex-col gap-6">
              <h4 className="text-base font-bold text-navy-900 dark:text-slate-150 border-b border-slate-200 dark:border-slate-800 pb-3 uppercase tracking-wider">Example: PDF Parser Pipeline</h4>
              <div className="flex flex-col gap-4 relative">
                <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded p-4 text-xs text-slate-500 dark:text-slate-400">
                  <span className="font-semibold text-navy-900 dark:text-slate-200 block mb-1">Step 1: Document Ingestion</span>
                  User uploads PDF invoice to secure portal bucket.
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded p-4 text-xs text-slate-500 dark:text-slate-400 ml-4 border-l-brand-500 dark:border-l-brand-400 border-l-4">
                  <span className="font-semibold text-navy-900 dark:text-slate-200 block mb-1">Step 2: LLM Token Mapping</span>
                  Intelligent API parses formatting schema to find sub-totals, items, and tax dates.
                </div>
                <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded p-4 text-xs text-slate-500 dark:text-slate-400">
                  <span className="font-semibold text-navy-900 dark:text-slate-200 block mb-1">Step 3: Database Ingestion</span>
      ...
      <section className="py-24 bg-slate-50 dark:bg-[#0c1321]/50 border-b border-slate-100 dark:border-slate-800/80 reveal-on-scroll">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-8 bg-white dark:bg-[#0d1321] border border-slate-200 dark:border-slate-800 p-8 rounded-2xl shadow-sm">
            {faqs.map((faq, idx) => (
              <div key={idx} className="space-y-2 border-b border-slate-100 dark:border-slate-800 last:border-0 pb-6 last:pb-0">
                <h4 className="text-base font-extrabold text-navy-900 dark:text-slate-150 flex items-start gap-2">
                  <span className="text-brand-500 dark:text-brand-400 font-mono font-bold">Q:</span>
                  {faq.q}
                </h4>
