        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col gap-24">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
            <div className="lg:col-span-7 flex flex-col gap-6">
              <span className="inline-flex self-start items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-800 uppercase tracking-widest shadow-sm">
                FOUNDER & ENGINEERING LEADERSHIP
              </span>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-navy-900 dark:text-white tracking-tight leading-[1.15]">
                Founder-led engineering.<br />
                <span className="text-brand-500 dark:text-brand-400">Built for real business problems.</span>
              </h2>
            ...
            >
              <div className="bg-white dark:bg-[#0d1321] border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group-hover:shadow-[0_8px_40px_rgb(0,0,0,0.08)] group-hover:border-slate-300 dark:group-hover:border-slate-700 transform group-hover:-translate-y-0.5 transition-all duration-500 flex flex-col h-full">
                <div className="absolute top-0 right-0 w-32 h-32 bg-brand-50 dark:bg-slate-900 rounded-full blur-3xl -mr-16 -mt-16 opacity-50 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
                <div className="relative z-10 p-8 sm:p-10 flex flex-col items-start gap-8 flex-1">
                  <div className="h-20 w-20 rounded-2xl bg-gradient-to-br from-navy-900 to-navy-800 dark:from-slate-900 dark:to-slate-950 flex items-center justify-center text-white text-2xl font-extrabold shadow-md transform group-hover:scale-105 transition-transform duration-500">
                    ST
                  </div>
                  <div className="space-y-2">
                    <h4 className="font-extrabold text-navy-900 dark:text-slate-100 text-2xl tracking-tight">Sumit Tiwari</h4>
                    <p className="text-brand-600 dark:text-brand-400 font-semibold text-sm tracking-wide uppercase">Founder & Engineering Lead</p>
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">KVYASH Technologies</p>
                  </div>
                  <div className="w-full h-px bg-slate-100 dark:bg-slate-800 my-2"></div>
                  <p className="text-slate-700 dark:text-slate-350 text-sm font-semibold tracking-wide">
                    Software &bull; AI &bull; Automation &bull; Digital Products
                  </p>
                </div>
                <div className="relative z-10 bg-slate-50/50 dark:bg-slate-900/40 border-t border-slate-100 dark:border-slate-800/60 px-8 py-4 sm:px-10 flex items-center justify-between mt-auto">
                  <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Founder Profile</span>
                  <span className="text-sm font-bold text-brand-600 dark:text-brand-400 flex items-center gap-1.5 transition-colors group-hover:text-brand-500 dark:group-hover:text-brand-350">
          ...
          <div className="flex flex-col gap-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                <div key={item.title} className="bg-white dark:bg-[#0d1321] border border-slate-100 dark:border-slate-800 rounded-xl p-6 shadow-sm hover:shadow-md hover:-translate-y-1 transform transition-all duration-300 flex flex-col gap-3 group">
                  <h4 className="font-bold text-navy-900 dark:text-slate-100 text-base group-hover:text-brand-500 dark:group-hover:text-brand-400 transition-colors duration-300">{item.title}</h4>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col gap-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div key={step.num} className="bg-white dark:bg-[#0d1321] border border-slate-200/60 dark:border-slate-800/80 rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-4 text-slate-50 dark:text-slate-900/10 font-black text-6xl group-hover:text-brand-50 dark:group-hover:text-brand-900/20 transition-colors duration-500 transform translate-x-2 -translate-y-2 pointer-events-none select-none">
                  {step.num}
                </div>
                <div className="relative z-10 flex flex-col gap-3">
                  <span className="text-brand-500 dark:text-brand-400 font-bold text-xs tracking-widest uppercase">{step.title}</span>
                  <p className="text-navy-900 dark:text-slate-200 font-medium text-sm leading-relaxed max-w-[85%]">{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-8 bg-white dark:bg-[#0d1321] border border-slate-200 dark:border-slate-800 rounded-2xl p-8 sm:p-10 shadow-premium flex flex-col gap-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
              {[
                ...
                <div key={point} className="flex items-center gap-3">
                  <CheckCircle className="h-5 w-5 text-brand-500 dark:text-brand-400 shrink-0" aria-hidden="true" />
                  <span className="font-bold text-navy-900 dark:text-slate-250 text-sm">{point}</span>
                </div>
              ))}
            </div>
            <div className="w-full h-px bg-slate-100 dark:bg-slate-800"></div>
            <p className="text-center text-slate-600 dark:text-slate-400 font-medium text-sm sm:text-base">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {values.map((val) => (
              <div
                key={val.title}
                className="bg-slate-50 dark:bg-[#0d1321] border border-slate-100 dark:border-slate-800 rounded-xl p-6 shadow-premium hover:shadow-premium-hover transition-premium duration-300 flex flex-col justify-between"
              >
                <div>
      ...
      <section className="py-24 bg-white dark:bg-[#090d16] border-b border-slate-100 dark:border-slate-800/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="flex flex-col gap-6">
              ...
              <ul className="flex flex-col gap-3 mt-2">
                {handoffStandards.map((std) => (
                  <li key={std} className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400">
                    <CheckCircle className="h-5 w-5 text-brand-500 dark:text-brand-400 shrink-0" aria-hidden="true" />
                    <span>{std}</span>
                  </li>
                ))}
            ...
            {/* Visual Box */}
            <div className="bg-slate-50 dark:bg-[#0d1321] border border-slate-100 dark:border-slate-800 rounded-xl p-8 shadow-premium flex flex-col gap-6">
              <h4 className="text-lg font-bold text-navy-900 dark:text-slate-100 border-b border-slate-200 dark:border-slate-800 pb-3">Engineering Rigor</h4>
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  <Code className="h-5 w-5 text-brand-500 dark:text-brand-400 shrink-0 mt-0.5" />
                  <div>
                    <span className="font-semibold text-navy-900 dark:text-slate-200 text-sm block">TypeScript Compilation</span>
                ...
                <div className="flex items-start gap-3">
                  <Shield className="h-5 w-5 text-brand-500 dark:text-brand-400 shrink-0 mt-0.5" />
                  <div>
                    <span className="font-semibold text-navy-900 dark:text-slate-200 text-sm block">Security Auditing</span>
                ...
                <div className="flex items-start gap-3">
                  <Hammer className="h-5 w-5 text-brand-500 dark:text-brand-400 shrink-0 mt-0.5" />
                  <div>
                    <span className="font-semibold text-navy-900 dark:text-slate-200 text-sm block">CI/CD Deployments</span>
                    <span className="text-slate-500 dark:text-slate-400 text-xs">Automated pipeline scripts verify code builds before moving to production edge servers.</span>
                  </div>
                </div>
              </div>
            </div>
